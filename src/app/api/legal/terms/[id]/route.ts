// PUT /api/legal/terms/[id] - Update Terms (admin only)
// DELETE /api/legal/terms/[id] - Delete Terms (admin only)
...
const putHandler = asyncHandler(
  async (request: NextRequest, context: { params: Promise<{ id: string }> }) => {
    ...
    if (!existing || existing.type !== 'TERMS') {
      throw new ApiError('Terms document not found', HttpStatus.NOT_FOUND);
    }
    ...
    return successResponse({ document: updated }, 'Terms document updated');
  }
);
...
const deleteHandler = asyncHandler(
  async (request: NextRequest, context: { params: Promise<{ id: string }> }) => {
    ...
    return successResponse({ id }, 'Terms document deleted');
  }
);
