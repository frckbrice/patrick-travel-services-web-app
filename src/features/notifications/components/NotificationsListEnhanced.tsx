...
// Enhanced notifications list with filters and rich UI
...
const notifConfig: Record<NotificationType, { icon: any; className: string }> = {
    CASE_STATUS_UPDATE: {
        icon: Briefcase,
...
const mockNotifications: Notification[] = [
    {
        ...
        message: 'Your student visa application is now under review',
        isRead: false,
    ...
    },
    {
        ...
        message: 'Your bank statement needs to be updated',
        isRead: true,
        ...
        <div className="space-y-6">
            ...
            {filtered.length === 0 ? (
                ...
                <div className="space-y-2">
                    {filtered.map((notif) => {
                        const Icon = notifConfig[notif.type].icon;
                        return (
                            <Card
                                key={notif.id}
                                className={cn(
                                    'hover:shadow-md transition-all cursor-pointer',
                                    !notif.isRead && 'border-l-4 border-l-primary bg-muted/30'
                                )}
                                onClick={() => notif.actionUrl && (window.location.href = notif.actionUrl)}
                            >
                                <CardContent className="pt-6">
                                    <div className="flex items-start gap-4">
                                        <div className={cn('p-3 rounded-lg', notifConfig[notif.type].className)}>
